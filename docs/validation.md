...
Three passes of xfstests' `fsx` on fresh files with fixed seeds (1, 2, 3), so a failure reproduces: mapped reads and writes on with a 4 MiB maximum file; the same with `-R -W` (no mapped I/O); and `-X`, which re-reads and compares the whole file after every operation, on the default 256 KiB file with a quarter of the operations. The shadow image and log go to the results directory (`-P`), not the mount, and a failure names its seed and leaves `<pass>.fsxlog`.
